...
int print_rd_kafka_conf_value(const rd_kafka_conf_t *conf, const char *key) {
  ...
  if ((ret = rd_kafka_conf_get(conf, key, NULL, &dest_sz)) != RD_KAFKA_CONF_OK)
    return ret;
...
int main(void) {
  ...
  rd_kafka_conf_res_t ret;
  if ((ret = rd_kafka_conf_set(conf, "bootstrap.servers", "localhost:9093", buf, ERR_BUF_SIZE)) != RD_KAFKA_CONF_OK) {
